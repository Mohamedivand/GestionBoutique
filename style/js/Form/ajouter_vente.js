...
let vente = {
    ...
    calculerMontantARemettre: () => {
        let res = vente.sommeRecue - vente.calculerTotalAvecRemise();
...
$("#formFinalVente").on("submit", function (e) {
    ...
    if (vente.sommeRecue < vente.calculerTotalAvecRemise()) {
    ...
    if (lancerRequest) {
        ...
        $.ajax({
            type: "POST",
            url: "../../api/vente/ajouter_vente.php",
            data: {
                token: 'djessyaroma1234',
                listeProduit: vente.chargerListeProduit(),
                typeVente: $("#typeVente").val(),
                reduction: vente.reduction,
                // montantPayer : $("#sommeRecue").val(),
                nom: client.nom,
                tel: client.num
            },
            dataType: "JSON",
            success: function (response) {
                $("#validerVenteBtn").attr("disabled", false);

                if (confirm("Vente effectuer. Voulez vous un recu?")) {
                    window.open(`../../print/${(response.caisseExiste == 1) ? "printRecu" : "printRecuA4"}.php?idVente=${response.idVente}`, '_blank');
                } else {
                    alert("la vente a ete enregistre avec succes !!!")
                }
