...
try {
    ...
    $query = $bdd->prepare('INSERT INTO vente (reduction, montantPayer, total_prix_produit, typeVente, id_boutique, id_user) VALUES(?,?,?,?,?,?)');
    $query->execute(array(
        $reduction,
        $sum_total_produit - $reduction,
        0,
        $typeVente,
        $boutique->getId(),
        $newClient->getId()
    ...
    if ($newVente->ajouterProduit($listeProduit)) {
        echo (json_encode(
            array(
                "idVente" => $newVente->getId(),
                "caisseExiste" => $boutique->getCaisseExiste()
            )
        ));
