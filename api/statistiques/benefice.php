...
try{
    ...
    if(isset($_COOKIE['chart2'])){
        $data = json_decode($_COOKIE['chart2'], true);
    ...
    for($i=1; $i<8; $i++){
        ...
        foreach($listeIdVente AS $idVente){
            ...
            if($date == $dateVente && $vente->getTotal_a_payer() <= $vente->getMontantPayer()){
                $montant += $vente->getBenefice();
            }
        }

        $response[]= array("jour" => $montant);
    }    
    setcookie("chart2", json_encode($response), time()+600, "/");
